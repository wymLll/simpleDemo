...
<template>
  <div class="cart">
    <div class="cart-header">
      <div class="cat-header-title">购物清单</div>
    ...
    <div class="cart-content">
      <div class="cart-content-main" v-for="(item, index) in cartList" :key="index">
        <div class="cart-info">
          <img :src="produceDictList[item.id].image">
          <span>{{produceDictList[item.id].name}}</span>
        </div>
        <div class="cart-price">￥{{produceDictList[item.id].cost}}</div>
        ...
        <div class="cart-cost">
            ￥{{produceDictList[item.id].cost * item.count}}
        </div>
...
export default {
  ...
  computed: {
    ...
    costAll() {
      let cost = 0;
      this.cartList.forEach(item => {
        cost += this.produceDictList[item.id].cost * item.count;
      });
      return cost;
    }
  },
